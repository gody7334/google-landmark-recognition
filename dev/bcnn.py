...
class sBCNN_CP(sBCNN):
    ...
    def bi_check_point(self):
        def forward(*x):
            bi_vector = self.com_bi_pool(x[0], x[0])
            bi_vector = self.bi_vector_bn(bi_vector)
            return bi_vector
        return forward
    ...
    def forward(self, x):
        ...
        f1 = f1.view(f1.size(0), -1)
        bi_vector = checkpoint.checkpoint(self.bi_check_point(), f1)
        fc = self.fc(bi_vector)
        return fc

class sBCNN_CP_HP(sBCNN_CP):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fc = self.fc.half()
